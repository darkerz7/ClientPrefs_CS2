...
namespace TestSharedPlugin
{
	public class TestSharedPlugin : BasePlugin
	{
		...
		public void OnSetCookie(CCSPlayerController? player, CommandInfo command)
		{
			if(_CP_api != null)
			{
				string sValue = command.GetArg(1);
				if (!string.IsNullOrEmpty(sValue)) _CP_api.SetClientCookie(player.SteamID.ToString(), "lang", sValue);
			}
		}
		[ConsoleCommand("cf_get", "Example of getting data from a database")]
		[CommandHelper(minArgs: 0, usage: "[]", whoCanExecute: CommandUsage.CLIENT_ONLY)]
		public void OnGetCookie(CCSPlayerController? player, CommandInfo command)
		{
			if (_CP_api != null)
			{
				string sValue = _CP_api.GetClientCookie(player.SteamID.ToString(), "lang");
				Server.NextFrame(() =>
				{
